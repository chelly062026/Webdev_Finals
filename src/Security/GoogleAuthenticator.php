            ...
            new UserBadge($accessToken->getToken(), function () use ($accessToken, $client) {
                ...
                if (!$user) {
                    ...
                    if (!$user) {
                        ...
                        $user = new User();
                        $user->setEmail($email);
                        $user->setUsername(preg_replace('/[^a-z0-9_\.\-]/i', '_', strstr($email, '@', true) ?: $email));
                        $user->setFirstName($googleUser->getFirstName() ?? '');
                        $user->setLastName($googleUser->getLastName() ?? '');
                        // Set a dummy password since it's a required field in most User entities
                        $user->setPassword(bin2hex(random_bytes(16)));
                        // Google OAuth users should be staff accounts per requirements
                        $user->setRoles(['ROLE_STAFF']);
                        $user->setIsVerified(true);
                    } elseif (!$this->hasRole($user, 'ROLE_STAFF')) {
                        throw new CustomUserMessageAuthenticationException('Only staff users can login with Google OAuth.');
                    }
